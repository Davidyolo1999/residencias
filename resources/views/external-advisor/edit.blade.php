            ...
            <div class="card-body">
                <form action="{{ route('externalAdvisor.update', $externaladvisor) }}" method="POST">
                    ...
                    <x-inputs.text-field-row name="email" label="Correo Electrónico:"
                        placeholder="Ingresé el Correo Electrónico" autofocus :default-value="$externaladvisor->user->email" />

                    {{-- FIRST NAME --}}
                    <x-inputs.text-field-row name="first_name" label="Nombre(s):" placeholder="Ingresé el Nombre:"
                        :default-value="$externaladvisor->first_name" />

                    {{-- FATHER'S LAST NAME --}}
                    <x-inputs.text-field-row name="fathers_last_name" label="Apellido Paterno:"
                        placeholder="Ingresé el Apellido Paterno" :default-value="$externaladvisor->fathers_last_name" />

                    {{-- MOTHERS'S LAST NAME --}}
                    <x-inputs.text-field-row name="mothers_last_name" label="Apellido Materno:"
                        placeholder="Ingresé el Apellido Materno" :default-value="$externaladvisor->mothers_last_name" />

                    {{-- CHARGE --}}
                    <x-inputs.text-field-row name="charge" label="Cargo:" placeholder="Ingresé el Cargo"
                        :default-value="$externaladvisor->charge" />

                    {{-- CAREER --}}
                    <x-inputs.text-field-row name="career" label="Carrera:" placeholder="Ingresé la Carrera"
                    ...
                    {{-- CURP --}}
                    <x-inputs.text-field-row name="curp" label="CURP:" placeholder="Ingresé el Curp" :default-value="$externaladvisor->curp" />

                    {{-- PHONE NUMBER --}}
                    <x-inputs.text-field-row name="phone_number" label="Teléfono:" placeholder="Ingresé Número de Teléfono"
                                ...
                                <select class="form-control" name="state_id" id="state_id">
                                    ...
                                    @foreach ($states as $state)
                                        <option value="{{ $state->id }}"
                                            @if ($state->id == old('state_id', $externaladvisor->state_id)) selected @endif>{{ $state->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('state_id')
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('externalAdvisor.updatePassword', $externaladvisor) }}" method="POST">
                    ...
                    @method('PUT')
                    {{-- PASSWORD --}}
                    <x-inputs.text-field-row name="password" label="Contraseña Nueva:"
                        placeholder="Ingresé la Contraseña Nueva" type="password" />
                    {{-- CONFIRM PASSWORD --}}
                    <x-inputs.text-field-row name="password_confirmation" label="Confirmar Contraseña:"
                        placeholder="Confirme la Contraseña Nueva" type="password" />
                    <div class="text-right">
                        <a href="{{ route('externalAdvisor.index') }}" class="btn btn-sm btn-warning mr-3">
                            <i class="material-icons">cancel</i><b> Cancelar</b> </a>
                        <button class="btn btn-sm btn-success"><i class="material-icons">save</i><b> Guardar</b></button>
        ...
        $(() => {
            $('#state_id').change((e) => {
                ...
                const municipalities = state
                    .locations
                    .map((municipality) =>
                        `<option value="${municipality.id}" ${municipality.id == @json(old('municipality_id', $externaladvisor->municipality_id)) ? 'selected' : ''}>${municipality.name}</option>`
                        )
                    .join('');
            ...
            $('#municipality_id').change((e) => {
                ...
                const localities = municipality
                    .locations
                    .map((locality) =>
                        `<option value="${locality.id}" ${locality.id == @json(old('locality_id', $externaladvisor->locality_id)) ? 'selected' : ''}>${locality.name}</option>`
                        )
                    .join('');
